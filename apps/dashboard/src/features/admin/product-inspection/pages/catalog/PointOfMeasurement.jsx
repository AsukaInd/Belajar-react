...
import apiPointOfMeasurement from "../../api/catalog/apiPointOfMeasurement";
...
function ProductSizes() {
   ...
   const deleteData = async (id) => {
      await apiPointOfMeasurement.deleteData(id).then(() => {
   ...
   const onSubmit = async (data) => {
      if (isEdit) {
         data._method = "PUT";
         await apiPointOfMeasurement.updateData(selected.id, data).then(() => {
            setRenderData(!renderData);
            setAddEditDialog(false);
            setIsEdit(false);
         });
      } else {
         await apiPointOfMeasurement.addData(data).then(() => {
            setRenderData(!renderData);
            setAddEditDialog(false);
         });
      }
   ...
   return (
      <>
         <TableInspection
            title="Point of Measurement"
            addBtn="Add Measurement"
            api={apiPointOfMeasurement}
