...
function SizeMeasurementTable({ control, register, size }) {
   const { fields, append, prepend, remove, swap, move, insert } =
      useFieldArray({
         ...
         name: "size_measurement", // unique name for your Field Array
      });
   ...
   return (
      <section>
               ...
               {fields.map((field, index) => (
                  <tr className="border-b-2">
                     ...
                     <td>
                        <input
                           {...register(
                              `size_measurement.${index}.pom.${size}`
                           )}
                           type="text"
                           className="border-1 border-gray-400 p-3  rounded w-full"
                        />
                     ...
                     <td>
                        <input
                           {...register(
                              `size_measurement.${index}.req.${size}`
                           )}
                           type="text"
                           className="border-1 border-gray-400 p-3  rounded w-full"
                        />
                     ...
                     <td>
                        <input
                           {...register(
                              `size_measurement.${index}.plus.${size}`
                           )}
                           type="text"
                           className="border-1 border-gray-400 p-3 rounded w-full"
                        />
                     ...
                     <td>
                        <input
                           {...register(
                              `size_measurement.${index}.minus.${size}`
                           )}
                           type="text"
                           className="border-1 border-gray-400 p-3 rounded w-full"
                        />
         ...
         <button
            type="button"
            onClick={() => append({ sizemeasurement: "", })}
            className="bg-blue-2  text-white p-2 border-0 rounded"
         >
            Add Data
         </button>
      </section>
